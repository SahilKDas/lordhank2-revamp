...
class IceSpiritMob extends Entity {
  static defaultDefinition = {
    ...
    speed: 35,
    damage: 12,
    attackCooldown: [0.35, 0.45],
    isBoss: false,
  };

  constructor(game, definition) {
    super(game, Types.Entity.IceSpirit, definition);

    this.isGlobal = this.definition.isBoss;
    this.shape = Circle.create(0, 0, this.size);
    this.angle = helpers.random(-Math.PI, Math.PI);
    this.coinsDrop = this.definition.isBoss ? 15000 : 1500;
    ...
    this.attackTimer = new Timer(0, this.definition.attackCooldown[0], this.definition.attackCooldown[1]);
